import { getFirestore, doc, getDoc, collection, addDoc, getCountFromServer } from "firebase-admin/firestore";
...
export class firebaseDatabaseConnector {
    ...
    async getDocumentCount(collName: string): number {
        return await db.collection(collName).get().then(function(querySnapshot) {
   ...
   async getTestFromId(testId: number): Promise<Test> {
    ...
    async getTestDataFromTestId(testId: number): TestData {
        console.log(testId)
        const testIdString: string = ""+testId
        const querySnapshot = await db.collection("testData").where("id", "==", testId).get()
    ...
    async getTestDataFromTestDatakey(testId: number, testDataKey:string): Promise<TestData> {
        const querySnapshot = await db.collection("testData").where("id", "==", testId).where("key", "==", testDataKey).get();
        if (querySnapshot.size > 0) {
            let testData: TestData = {data: []};;
            querySnapshot.forEach((doc) => {
                testData.data.push(doc.data());
    ...
    async getLatestTestDataFromTestDatakey(testId: number, testDataKey:string): Promise<TestDataEntry> {
        const querySnapshot = await db.collection("testData").where("id", "==", testId).where("key", "==", testDataKey).get();
        if (querySnapshot.size > 0) {
            let testData: TestData = {data: []};;
    ...
    async getTestLogFromTestId(testId: number): Promise<TestLog> {
        const testIdString: string = ""+testId
        const querySnapshot = await db.collection("logs").where("id", "==", testId).get()
    ...
    async getTestLogFromKey(testId: number, key: string): Promise<TestLog> {
        ...
        const querySnapshot = await db.collection("logs").where("id", "==", testId).where("key", "==", key).get()
