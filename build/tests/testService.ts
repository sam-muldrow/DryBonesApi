...
export const returnThisNumber = async (id: number): Promise<number> => {
  return id;
};
...
export async function getTestDataFromTestDatakey(
  testId: number,
  TestDataKey: string
): Promise<TestData> {
  return new Promise<TestData>(async (resolve) => {
    resolve(
      await firbaseConnector.getTestDataFromTestDatakey(testId, TestDataKey)
    );
  });
}

export async function getLatestTestDataFromTestDatakey(
  testId: number,
  TestDataKey: string
): Promise<TestDataEntry> {
  return new Promise<TestDataEntry>(async (resolve) => {
    resolve(
      await firbaseConnector.getLatestTestDataFromTestDatakey(
        testId,
        TestDataKey
      )
    );
...
export async function getTestLogFromKey(
  testId: number,
  TestLogKey: string
): Promise<TestLog> {
  return new Promise<TestLog>(async (resolve) => {
...
export async function AddOrEditTestData(
  testId: number,
  key: string,
  value: string,
  relatedTo: string
  ...
  return new Promise<Number>(async (resolve) => {
    resolve(
      await firbaseConnector.AddOrEditTestData(testId, key, value, relatedTo)
    );
  });
}

export async function AddOrEditLogEntryToTestId(
  testId: number,
  key: string,
  value: string
): Promise<Number> {
  return new Promise<Number>(async (resolve) => {
    resolve(
      await firbaseConnector.AddOrEditLogEntryToTestId(testId, key, value)
    );
  });
}
